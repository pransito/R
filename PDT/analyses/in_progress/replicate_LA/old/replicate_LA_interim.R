...
if (with_ed) {
  mod_la      = glmer(accept_reject ~ gain + loss + ed_abs + (gain + loss + ed_abs|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)
  mod_lag     = glmer(accept_reject ~ (gain + loss + ed_abs)*HCPG + (gain + loss + ed_abs|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)
  mod_lag_s   = 'glmer(accept_reject ~ (gain + loss + ed_abs)*HCPG + (gain + loss + ed_abs|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)'
} else {
...
la_coefs = agk.get.compl.coef(mod_lag,'HCPG')
...
if (with_ed) {
  mod_la      = glmer(accept_reject ~ (gain + loss + ed_abs)*smoking_ftdt + (gain + loss + ed_abs|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)
  mod_lag     = glmer(accept_reject ~ (gain + loss + ed_abs)*smoking_ftdt*HCPG + (gain + loss + ed_abs|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)
  mod_lag_s   = 'glmer(accept_reject ~ (gain + loss + ed_abs)*smoking_ftdt*HCPG + (gain + loss + ed_abs|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)'
} else {
  mod_la      = glmer(accept_reject ~ (gain + loss)*smoking_ftdt + (gain + loss|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)
  mod_lag     = glmer(accept_reject ~ (gain + loss)*smoking_ftdt*HCPG + (gain + loss|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)
  mod_lag_s   = 'glmer(accept_reject ~ (gain + loss)*smoking_ftdt*HCPG + (gain + loss|subject), data = data_pdt, family = binomial,nAGQ = 0, control = cur_control)'
}
...
if (eval_res) {
  ...
  obs_fixef <- fun_extract(mod_lag)
  1-agk.density_p(x_la_HCgrPG,obs_fixef[9])
  
  # p-value for PG gr HC: beta gain (one-sided)
  agk.density_p(con_bg_PG_HC,obs_fixef[4]-obs_fixef[3])
  
  # p-value for PG gr HC: beta loss (one-sided)
  1-agk.density_p(con_bl_PG_HC,obs_fixef[6]-obs_fixef[5])
  
  # p-value for PG gr HC: beta loss (one-sided)
  1-agk.density_p(con_an_PG_HC,obs_fixef[2]-obs_fixef[1])
}
