...
# no mu models: can be linearized (Charpentier) or are overidentified
if (add_nonlinear_behav) {
  # load non-linear models (beta_gain and beta_loss as exponents)
  ## TODO: REDO the non-linear bit after review; https://de.wikipedia.org/wiki/Verlustaversion
  warning('REDO the non-linear bit after review; https://de.wikipedia.org/wiki/Verlustaversion')
  cur_home = getwd()
  setwd('..')
  setwd('model_calculation/')
  all_files = dir()
  all_files = all_files[grep(which_study,all_files)]
  lanlec = all_files[grep(paste0('lanlec',which_study),all_files)]
  lanle  = all_files[grep(paste0('lanle',which_study),all_files)]
  load(lanle)
  row.names(all_params) = all_params$VPNR
  all_params$VPNR = NULL
  fm$lanle = all_params
  load(lanlec)
  row.names(all_params) = all_params$VPNR
  all_params$VPNR = NULL
  fm$lanlec = all_params
  setwd(cur_home)
  ## TODO
}
