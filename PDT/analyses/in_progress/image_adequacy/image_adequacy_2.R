...
if (do_ratings == 1) {
  ...
  ravc              = lmer(valence ~ (cat) + (cat| subject) + (cat|stim),data = data_pdt, REML = F, nAGQ = 0,control=cur_control)
  ...
  raac              = lmer(arousal ~ (1+cat) + (0+cat| subject) + (cat|stim),data = data_pdt,REML = F)
  raacg             = lmer(arousal ~ (1+cat)*HCPG + (0+cat| subject) + (cat|stim),data = data_pdt,REML = F)
