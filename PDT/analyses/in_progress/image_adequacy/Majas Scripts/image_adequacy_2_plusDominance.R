...
if (do_ratings == 1) {
  ...
  rarg0          = lmer(imageRating2s ~ 1 + (1| subject) + (1|stim),data = data_pdt,REML=F,nAGQ = 0,control=cur_control) # representative of gamble
  rargc          = lmer(imageRating2s ~ (cat) + (cat| subject) + (cat| stim),data = data_pdt,REML=F,nAGQ = 0,control=cur_control) # representative of gamble
  rargcg         = lmer(imageRating2s ~ (cat*HCPG) + (cat| subject) + (cat| stim),data = data_pdt,REML=F,nAGQ = 0,control=cur_control) # representative of gamble
  ...
  cons2 = glht(rargc, linfct = mcp(cat = "Tukey"))
