...
if (do_ratings == 1) {
  ...
  ravc              = lmer(valence ~ (cat) + (cat| subject) + (1|stim),data = data_pdt, REML = F, nAGQ = 0,control=cur_control)
  lmList(valence ~ cat| subject,data = data_pdt)
  ravcg             = lmer(valence ~ (cat*HCPG) + (cat|subject) + (1|stim),data = data_pdt,REML = F,nAGQ = 0,control=cur_control)
  ...
  cons = glht(ravc, linfct = mcp(cat = "Tukey"))
  ...
  raac              = lmer(arousal ~ (0+cat) + (0+cat| subject) + (1|stim),data = data_pdt,REML = F)
  lmList(arousal ~ 0+cat| subject,data = data_pdt)
  raacg             = lmer(arousal ~ (cat)*HCPG + (cat| subject) + (1|stim),data = data_pdt,REML = F)
  ...
  radc              = lmer(dominance ~ (0+cat) + (0+cat| subject) + (1|stim),data = data_pdt, REML = F, nAGQ = 0,control=cur_control)
  radc_PG          = lmer(dominance ~ cat + (cat| subject) + (1|stim), data = data_pdt_PG,REML=F,control=cur_control)       
  radc_HC          = lmer(dominance ~ cat + (cat| subject) + (1|stim), data = data_pdt_HC,REML=F,control=cur_control)
  radcg             = lmer(dominance ~ (cat*HCPG) + (cat|subject) + (1|stim),data = data_pdt,REML = F,nAGQ = 0,control=cur_control)
  ...
  # additional contrasts needed
  cons = glht(radc_PG, linfct = mcp(cat = "Tukey"))
  summary(cons,test=adjusted('none'))
  
  cons2 = glht(radc_HC, linfct = mcp(cat = "Tukey"))
  summary(cons,test=adjusted('none'))
  ...
  rargc          = lmer(imageRating2s ~ (0+cat) + (0+cat| subject) + (1| stim),data = data_pdt,REML=F) # representative of gamble
  rargcg         = lmer(imageRating2s ~ (cat*HCPG) + (cat| subject) + (1| stim),data = data_pdt,REML=F) # representative of gamble
  ...
  # additional contrasts needed
  cons = glht(rargc, linfct = mcp(cat = "Tukey"))
  summary(cons,test=adjusted('none'))
  ...
  rarcc             = lmer(imageRating1s ~ (cat) + (cat| subject) + (1|stim), data = data_pdt,REML=F,control=cur_control)       # craving
  rarcc_PG          = lmer(imageRating1s ~ cat + (cat| subject) + (1|stim), data = data_pdt_PG,REML=F,control=cur_control)       # craving
  rarcc_HC          = lmer(imageRating1s ~ cat + (cat| subject) + (1|stim), data = data_pdt_HC,REML=F,control=cur_control)       # craving
  rarccg            = lmer(imageRating1s ~ (cat*HCPG) + (cat| subject) + (1|stim),data = data_pdt,REML=F,control=cur_control) # craving
  ...
  rarpc             = lmer(imageRating4s ~ (0+cat) + (0+cat| subject) + (1| stim),data = data_pdt,REML=F) # representative of gamble
  rarpcg            = lmer(imageRating4s ~ (cat)*HCPG + (cat| subject) + + (1| stim),data = data_pdt,REML=F) # representative of gamble
  ...
  # additional contrasts needed
  cons = glht(rarpc, linfct = mcp(cat = "Tukey"))
  summary(cons,test=adjusted('none'))
  ...
  if (which_study == "sanity" & standard_contrast == 0) {
    ...
    rownames(cur_con) = c("neu","neg","pos")    contrasts(data_pdt$cat) = cur_con
  } else if (which_study == "Prestudy" & standard_contrast == 0) {
    cur_con           = matrix(c(-1,1,0,0,1,-1),nrow = 3,byrow = F)
  ...
  rarnc             = lmer(imageRating3s ~ (0+cat) + (0+cat|subject) + (1|stim),data = data_pdt,REML=F) # representative of neg
  rarnc_PG          = lmer(imageRating3s ~ cat + (cat| subject) + (1|stim), data = data_pdt_PG,REML=F,control=cur_control)       #representative of neg 
  rarnc_HC          = lmer(imageRating3s ~ cat + (cat| subject) + (1|stim), data = data_pdt_HC,REML=F,control=cur_control)       #representative of neg 
  rarncg            = lmer(imageRating3s ~ (cat)*HCPG + (cat| subject) + (1|stim),data = data_pdt,REML=F) # representative of neg
  ...
  cons3 = glht(rarnc_HC, linfct = mcp(cat = "Tukey"))
  summary(cons3,test=adjusted('none'))  
  ...
  ramqc             = lmer(imageRating5s ~ (cat) + (cat| subject) + (1|stim),data = data_pdt_PG,REML=F) 
  ramqcg            = lmer(imageRating5s ~ (cat)*HCPG + (cat| subject) + (1|stim),data = data_pdt_PG,REML=F) 
  anova(ramq0,ramqc)
