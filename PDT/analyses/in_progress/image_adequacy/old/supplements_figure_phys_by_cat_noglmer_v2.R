...
cr_agg_ra = crdf
cr_agg_ra$subject = row.names(cr_agg_pp)
...
if (plot_and_stats) {
  ...
  for (dd in 1:length(des_vars)) {
    ...
    # formulas
    cur_form_00 = paste0(des_vars[dd],' ~ gain + loss + (gain + loss | subject) + (gain +loss | stim)')
    cur_form_c0 = paste0(des_vars[dd],' ~ gain + loss + cat + (gain + loss + cat | subject) + (gain + loss | stim)')
    cur_form_cg = paste0(des_vars[dd],' ~ (gain + loss + cat)*HCPG + (gain + loss + cat | subject) + (gain + loss | stim)')
